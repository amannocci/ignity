...
@test "check default service had run" {
  result=$(echo 'test -d /etc/ignity' | docker::run)
...
@test "check no-start service at build time can be started" {
  result=$(echo 'rm -f /run/ignity/services-state/no-start-at-build-time/down && s6-svc -u /run/ignity/services-state/no-start-at-build-time' | docker::run)
  echo "${result}" | grep "no-start service at build time"
  [ "$?" -eq 0 ]
}

@test "check no-start service at runtime can be started" {
  result=$(echo 'rm -f /run/ignity/services-state/no-start-at-runtime/down && s6-svc -u /run/ignity/services-state/no-start-at-runtime' | docker::run)
