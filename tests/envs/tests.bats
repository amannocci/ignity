...
@test "check environment variable from envfile is present" {
  echo 'with-env env | grep "FOOBAR=test"' | docker::run
  [ "$?" -eq 0 ]
}

@test "check environment variable from envfile is overridable" {
  echo 'with-env env | grep "FOOBAR=override"' | DOCKER_ARGS="${DOCKER_ARGS} -e FOOBAR=override" docker::run
  [ "$?" -eq 0 ]
}

@test "check environment variable from envfile are overridable in order" {
  echo 'with-env env | grep "INHERIT=foobar"' | docker::run
  [ "$?" -eq 0 ]
}
